...
def compute_metrics(X, Y, nearest_k=5, model=None, distance=None):
    ...
    alphas, alpha_precision_curve, beta_coverage_curve, Delta_precision_alpha, Delta_coverage_beta, a = compute_alpha_precision(
        X_out, Y_out, model.c)
    # print(a)
    results['Dpa'] = Delta_precision_alpha
    results['Dcb'] = Delta_coverage_beta
    # results['mean_aut'] = np.mean(authen)
    # I don't know if this would change the program intent
    results['mean_aut'] = np.mean(a)
...
def alaa_compare(X, Y, distance=None):
    model = None
