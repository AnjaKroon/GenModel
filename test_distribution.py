        ...
        for U in list_U:
            ...
            for B in tqdm(Bs):  # For each bin granularity
                ...
                def compile_all_stats(all_samples_list, all_stat_list,baseline_all_stat_list,   U,  B): 
                    ...
                    random_stat = [i['B_random'] for i in S_trials_for_this_B_list]
                    all_stat_list.append(algo_stat)
                    baseline_all_stat_list.append(random_stat)

                compile_all_stats(ground_truth_samples_list, stat_uni,stat_uni_baseline, U,B=B) # compile stats for ground truth
                compile_all_stats(tempered_samples_list, stat_temper,stat_temper_baseline,U,B=B) # compile stats for tempered
                compile_all_stats(mid_tempered_samples_list,stat_mid_temper,stat_mid_temper_baseline, U,B=B) # compile stats for tempered
                compile_all_stats(easy_tempered_samples_list,stat_easy_temper,stat_easy_temper_baseline, U, B=B) # compile stats for tempered
            ...
            put_on_plot(Bs, {'easy tempered random': stat_easy_temper_baseline})
